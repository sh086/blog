...
export default defineConfig({
  ...
  themeConfig: {
    siteTitle: true,  // 是否展示站点标题,默认true
  }
})
```
...
export default defineConfig({
    // 社交链接
    // 可填入的icon有: gitlab、tiktok、twitter、wechat、qq、google
    // facebook、instagram、linkedin、mastodon、slack、x、discord
    socialLinks: [
      { icon: 'bilibili', link: '...' },
      { icon: 'youtube', link: '...' },
      { icon: 'github', link: '...' },
...
​	　打开[这里](https://shields.io/badges)，在输入框中输入 `any text-you like-blue`，点击生成按钮即可生成链接。该链接格式从左至右由**标签、消息、颜色**组成，其中标签可以不写。PS：链接也可参考[这里](https://github.com/Envoy-VC/awesome-badges) 或 [这里](https://github.com/Ileriayo/markdown-badges)。

![image-20250305134653727](./images/image-20250305134653727.png)
