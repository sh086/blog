...
![image-20260128162753884](images/image-20260128162753884.png)
...
![image-20260128164254825](images/image-20260128164254825.png)
...
![image-20260129140623333](images/image-20260129140623333.png)
...
![image-20260128175506440](images/image-20260128175506440.png)

​	　若构建中提示错误，使用自带的AI分析可解决问题。部署完成后，在`制品`Tab栏可以看到生成的镜像。

![image-20260129170835477](images/image-20260129170835477.png)
...
![image-20260129134909491](images/image-20260129134909491.png)



### 3、API和TASK分离部署

​	　对于一些复杂的部署，如需要生产环境的API和TASK分离部署的，我们可以通过构建镜像时，使用不同的`Dockerfile`来控制。
...
​	　通过给`appname`注入一个不存在的执行器名称，即可实现定时任务不被xxl-job调用的效果。
...
![image-20260128180253143](images/image-20260128180253143.png)
...
![image-20260128180827640](images/image-20260128180827640.png)

​	　第一点就是，首次是需要新增K8S访问CNB制品库的访问令牌，迁移第二个项目时就可使用改令牌无需再次新增了。注意，生产和测试第一次都是需要新增的。

![image-20260128181456369](images/image-20260128181456369.png)

​	　第二点就是，需要选择`镜像地址`为CNB的新镜像地址和镜像版本，镜像地址和镜像版本可以在`CNB`的`制品`中查看。配置完成后，点击`更新配置`按钮，等K8S重新部署后即可完成容器部署。

![image-20260129143217140](images/image-20260129143217140.png)
